...
const Navbar = () => {
  ...
  return (
    <div className='flex justify-between items-center text-sm py-4 mb-5 border-b border-b-gray-400'>
        <img onClick={() => navigate('/')} className='cursor-pointer w-36 h-20' src={assets.logo2} alt='' />
        ...
        <div className='flex items-center gap-4'>
                ...
                <div className='flex items-center gap-2 cursor-pointer group relative'>
                    ...
                    <div className='absolute top-0 right-0 pt-14 text-base font-medium text-gray-600 z-20 hidden group-hover:block'>
                        <div className='min-w-48 bg-stone-100 rounded flex flex-col gap-4 p-4'>
                            ...
                            <p onClick={() => setToken(false)} className='hover:text-black cursor-pointer'>Louout</p>
                        </div>
                    </div>
                </div> 
                : 
            ...
            {/*----------- mobile menu ------------- */}
            <div className={`${showMenu ? 'w-full fixed' : 'h-0 w-0'} md:hidden right-0 top-0 bottom-0 z-20 overflow-hidden bg-white transition-all duration-300`}>
                <div className='flex items-center justify-between px-5 py-6'>
                    <img className='w-36' src={assets.logo} alt='logo' />
                    <img onClick={() => setShowMenu(false)} className='w-7 cursor-pointer'  src={assets.cross_icon} alt='cross_icon' />
                 </div>
                 <ul className='flex flex-col items-center gap-2 mt-5 text-lg font-medium'>
                    ...
                    <NavLink className='px-4 py-2 rounded inline-block' onClick={() => setShowMenu(false)} to={'/contacts'}><p className='px-4 py-2 rounded inline-block'>CONTACT</p></NavLink>
                 </ul>
            </div>
        </div>
    </div>
